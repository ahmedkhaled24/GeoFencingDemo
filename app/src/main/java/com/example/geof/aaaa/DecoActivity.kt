...
import com.example.geof.BuildConfig
import com.example.geof.GeofenceBroadcastReceiver
...
class DecoActivity : AppCompatActivity(), OnCompleteListener<Void?> {
    ...
    private fun addGeofences() {
        ...
        mGeofencingClient!!.addGeofences(geofencingRequest, geofencePendingIntent).addOnCompleteListener(this)
    }
    ...
    private fun populateGeofenceList() {
        ...
        val dGeoArr: MutableList<Geo> = ArrayList()

        dGeoArr.add(Geo("Nassagoun", LatLng(30.1048863,31.3717017)))
        dGeoArr.add(Geo("Momentum", LatLng(30.1026439,31.371483)))
        dGeoArr.add(Geo("RadissonBlue", LatLng(30.1055824,31.3735042)))

        for (i in 0 until dGeoArr.size){
            mGeofenceList.add(
                Geofence.Builder() // Set the request ID of the geofence. This is a string to identify this
                    // geofence.
                    .setRequestId(dGeoArr[i].id) // Set the circular region of this geofence.
                    .setCircularRegion(
                        dGeoArr[i].latLong.latitude,
                        dGeoArr[i].latLong.longitude,
                        100f
    ...
    private val geofencesAdded: Boolean
        get() = PreferenceManager.getDefaultSharedPreferences(this).getBoolean(Constants.GEOFENCES_ADDED_KEY, false)
    ...
    private fun requestPermissions() {
        ...
        if (shouldProvideRationale) {
            ...
            showSnackbar(R.string.permission_rationale, R.string.ok) {
                // Request permission
                ActivityCompat.requestPermissions(this@DecoActivity,
...
data class Geo(val id: String, val latLong: LatLng)
